...
.factory(
		...
		function($http) {
			...
			factory.createCarResult = function() {
				return $http
						.get("xml/CreateCarsResponse.xml");
			}
...
/*	$http.get('json/car.json').success(function(response) {
	...
	function car() {
		...
		CreateCar.createCarResult().success(function(response) {
			createCarResults = x2js.xml_str2json(response);
			console.log(createCarResults.response);
			$scope.createCarResponse = createCarResults.response;
		});
	}
